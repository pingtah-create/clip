...
4. **[reframe.py](backend/reframe.py)** — two-pass face-tracked 9:16 crop. Pass 1: mediapipe face detection every `DETECT_EVERY` frames → list of face center-x → EMA smoothing. Pass 2: OpenCV reads the segment, crops per frame, pipes raw bgr24 frames to a single ffmpeg subprocess that encodes silent h264. Then `_mux_audio()` muxes the original audio back. Choosing OpenCV+pipe over pure-ffmpeg `sendcmd` is intentional — per-frame crop expressions are painful and mediapipe is in the loop anyway.
